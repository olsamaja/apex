...
final class AppViewModel_ReduceTests: XCTestCase {
    ...
    func testReduceIdle() throws {
        ...
        let events: [AppViewModel.Event] = [.onLoaded(Constants.header, Constants.rows),
    ...
    func testReduceLoading() throws {
        ...
        XCTAssertEqual(AppViewModel.reduce(.loading, .onLoaded(Constants.header, Constants.rows)), .loaded([Constants.header] + [Constants.rows]))
    ...
    func testReduceLoaded() throws {
        ...
        let events: [AppViewModel.Event] = [.onAppear,
                                            .onLoaded(Constants.header, Constants.rows),
                                            .onFailedToLoadData(DataError.invalidRequest)]
